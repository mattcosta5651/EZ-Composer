	...
	public AbstractAction ExportFile() throws IOException{
		class exportFile extends AbstractAction{
			...
			public void actionPerformed(ActionEvent action){
				...
				if(retval == JFileChooser.APPROVE_OPTION){
					try{
						//File newFile = new File(export.getSelectedFile().getName());//get project name
						//Write.midi(builder.getComposer().getProject().getPart(), newFile);
						builder.getComposer().getNotate().saveMidi();
					}
					catch(Exception e){
						e.printStackTrace();
					}						
