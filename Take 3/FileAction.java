			...
			public void actionPerformed(ActionEvent action){
				...
				if(retval == JFileChooser.APPROVE_OPTION){
					...
					try{
						...
						builder.getComposer().setProject(new Project((Project) ois.readObject(), builder.getComposer()));
					}
					catch(Exception e){
						e.printStackTrace();
					}
	...
	public AbstractAction ExportFile() throws IOException{
		class exportFile extends AbstractAction{
			...
			public void actionPerformed(ActionEvent action){
				...
				export.setApproveButtonMnemonic(KeyEvent.VK_ENTER);
				int retval = export.showSaveDialog(builder.getComposer().getGUI());
				
				if(retval == JFileChooser.APPROVE_OPTION){
					try{
						Write.midi(builder.getComposer().getProject().getPart(), export.getSelectedFile().getName());
					}
					catch(Exception e){
						e.printStackTrace();
					}						
