...
public class Notate extends JPanel /*implements 
                ActionListener, 
                WindowListener, JMC */{
    private Score score;
    //private Stave stave;
 ...
 //       this.show();
    ...
    private void makeAppropriateStaves(){
        ...
        for(int i=0; i<score.size(); i++) {
            ...
            if(currentPhrase.getHighestPitch() < jm.constants.Pitches.A6 &&
                currentPhrase.getLowestPitch() > jm.constants.Pitches.FS3) tempStaveArray[i] = new TrebleStave();
            else if(currentPhrase.getHighestPitch() < jm.constants.Pitches.F4 &&
                currentPhrase.getLowestPitch() > jm.constants.Pitches.B1) tempStaveArray[i] = new BassStave();
            else if(currentPhrase.getHighestPitch() > jm.constants.Pitches.A6 ||
                currentPhrase.getLowestPitch() < jm.constants.Pitches.B1) tempStaveArray[i] = new GrandStave();
        }
...
this.add(staveArray[i]);
 ...
 //       Toolkit tk = Toolkit.getDefaultToolkit();
 //       Dimension d = tk.getScreenSize();
//        setPreferredSize(new Dimension(this.width, Math.min(d.height-40, totalHeight+40)));
...
/*    public void actionPerformed(ActionEvent e){
 ...
 /*       else if(e.getSource() == setParameters) { // only works on the top stave
    ...
    */
     public void openMidi(Component parent) {
        Score s = new Score();
        JFileChooser loadMidi = new JFileChooser();
        int open = loadMidi.showOpenDialog(parent);
        String fileName = loadMidi.getName(loadMidi.getSelectedFile());
        if (fileName != null && open == JFileChooser.APPROVE_OPTION) {
            lastFileName = fileName;
            lastDirectory = loadMidi.getCurrentDirectory().getName();                        
     ...
     */
     ...
     public void openJM(Component parent) {
        JFileChooser loadjm = new JFileChooser();
        int open = loadjm.showOpenDialog(parent);
        String fileName = loadjm.getName(loadjm.getSelectedFile());
        
        if (fileName != null && open == JFileChooser.APPROVE_OPTION) {
           Score s = new Score();
            lastDirectory = loadjm.getCurrentDirectory().getName();  
            Read.jm(s, lastDirectory + loadjm.getSelectedFile().getName());
            setNewScore(s);
        }
    }
     ...
     */
     ...
     public void openJMXML(Component parent) {
        JFileChooser loadjmxml = new JFileChooser();
        int open = loadjmxml.showOpenDialog(parent);
        String fileName = loadjmxml.getName(loadjmxml.getSelectedFile());
        if (fileName != null && open == JFileChooser.APPROVE_OPTION) {
            Score s = new Score();
            lastDirectory = loadjmxml.getCurrentDirectory().getName(); 
     ...
     */
    public void saveMidi(Component parent) {
        JFileChooser fd = new JFileChooser();
        int save = fd.showSaveDialog(parent);
                            
        //write a MIDI file and stave properties to disk
        if (save == JFileChooser.APPROVE_OPTION) {
            Write.midi(score, fd.getCurrentDirectory().getName()+fd.getSelectedFile().getName());
            /*
            for(int i=0; i<staveArray.length; i++){
     ...
     */
    public void saveJM(Component parent) {
        JFileChooser fd = new JFileChooser();
        int save = fd.showSaveDialog(parent);
                            
        //write a MIDI file to disk
        if (save == JFileChooser.APPROVE_OPTION) {
            Write.jm(score, fd.getCurrentDirectory().getName()+fd.getSelectedFile().getName());
        }
    }
     ...
     */
    public void saveJMXML(Component parent) {
        JFileChooser fd = new JFileChooser();
        int save = fd.showSaveDialog(parent);
                            
        //write an XML file to disk
        if (save == JFileChooser.APPROVE_OPTION) {
            Write.xml(score, fd.getCurrentDirectory().getName()+fd.getSelectedFile().getName());
        }
    }
    ...
    */
    public Phrase readMidiPhrase(Component parent) {
        JFileChooser loadMidi = new JFileChooser();
        int open = loadMidi.showOpenDialog(parent);
        String fileName = loadMidi.getSelectedFile().getName();
        Phrase phr = new Phrase(0.0);
        Score scr = new Score();
        if (fileName != null && open == JFileChooser.APPROVE_OPTION) {
            Read.midi(scr, loadMidi.getCurrentDirectory().getName() + fileName); 
        }
        scr.clean();
